...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        b = len(source_lengths)
        max_str = source_lengths[0]

        # X = torch.zeros((max_str, b, self.model_embeddings.embed_size))
        #
        # for i_b in range(b):
        #     for i_s in range(source_lengths[i_b]):
        #         X[i_s][i_b] = self.model_embeddings.source(source_padded[i_s][i_b])
        ...
        assert enc_hiddens.shape == (b, max_str, self.hidden_size * 2)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        assert enc_hiddens_proj.shape == (batch_size, src_len, self.hidden_size)
